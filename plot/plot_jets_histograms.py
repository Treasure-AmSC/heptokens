...
#TRACK_FEATURES = TRACK_FEATURES[0:2]
...
def make_histograms(
    df: pd.DataFrame, output_dir: Path, bins: int, density: bool, dpi: int, dataset_name: str
) -> None:
    ...
    for feature, series in iter_features(filtered):
        ...
        if np.isfinite(ymin_positive) and ymin_positive > 0 and ymax > 0:
            ax.set_ylim(bottom=ymin_positive / 1.2, top=ymax * 3.)
...
def plot_track_histograms(
    ...
    """Stream track features and plot histograms split by jet flavour."""
    try:
        import h5py
    except ImportError as exc:
        raise ImportError(
            "h5py is required to read the tracks table but is not installed."
        ) from exc
