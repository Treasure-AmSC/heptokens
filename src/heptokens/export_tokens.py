...
class MemmapPredictionWriter(BasePredictionWriter):
    ...
    def __init__(self, tmp_dir: str, total_jets: int, num_elements: int, num_quantizers: int):
...
def export(cfg: DictConfig) -> None:
    ...
    with tempfile.TemporaryDirectory(dir=tmp_base) as tmp_dir:
        writer = MemmapPredictionWriter(tmp_dir, total_jets, num_elements, num_quantizers)
